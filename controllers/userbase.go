...
func (this *UserBaseController) Prepare() {
	...
	if token == "" || useridstr == "" {
		this.Data["json"] = map[string]interface{}{"code": 2, "message": "登录效验失败-效验数据为空", "data": ""}
		this.ServeJSON()
	}
	...
	if err != nil {
		this.Data["json"] = map[string]interface{}{"code": 2, "message": "登录效验失败-获取用户信息失败", "data": ""}
		this.ServeJSON()
	} else {
		if U.Token != token {

			this.Data["json"] = map[string]interface{}{"code": 2, "message": "登录效验失败-token不一致", "data": ""}
			this.ServeJSON()
		} else {
