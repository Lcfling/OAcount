$(function(){
	...
	*/
	...
	$('.js-mission-single').on('click', function(){
		var that = $(this);
		var status = that.attr('data-status')
		var id = that.attr('data-id');
		$.post('/mission/api/status', { status: status, id: id },function(data){
			dialogInfo(data.message)
			if (data.code) {
	...
	$('#area-form').validate({
		...
		messages : {
			name:{required: '请填写名称'},
			owner:{required: '指定责任人'},
			locations:{required: '请填写坐标信息'},
			coler:{required: '请设定区域颜色'}
		},
		submitHandler:function(form) {
			$(form).ajaxSubmit({
				type:'POST',
	...
	$('#missionmy-form').validate({
		...
		messages : {
			feedback:{required: '请填写文档名称'},
			detail:{required: '请选择类型'}
		},
		submitHandler:function(form) {
			$(form).ajaxSubmit({
				type:'POST',
				...
				success:function(data) {
					dialogInfo(data.message)
					if (data.code) {
						setTimeout(function(){window.location.href="/mymission/manage/"}, 2000);
					} else {
						setTimeout(function(){ $('#dialogInfo').modal('hide'); }, 1000);
					}
				}
